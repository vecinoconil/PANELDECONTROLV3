...
SUM_H    = 58 * mm            # bloque resumen/pie
...
def query_factura_data(empresa: Empresa, fac_id: int, cli_codigo: int) -> dict:
    ...
    try:
        ...
        cab['fax'] = (cab.get('cli_fax') or '').strip()
        cab['tipo_documento'] = {
...
def _draw_summary(c: rl_canvas.Canvas, cab: dict, venc: list, emp: dict,
    ...
    txt_reg = (emp.get('txt_registro') or '').strip()
    if txt_reg:
        _t(c, ML, y, txt_reg, size=6, color=colors.Color(0.35, 0.35, 0.35))
        y -= 9
    ...
    parts = list(filter(None, [
        ...
        emp.get('cpostal', ''),
        emp.get('localidad', ''),
        f"Tlf.- {emp.get('telefono1', '')}" if emp.get('telefono1') else '',
        emp.get('email', '') or emp.get('txt_2', '') or '',
    ]))
    bar_txt = ' - '.join(parts)
    if bar_txt:
        _t(c, ML, y, bar_txt, size=6, align='center', maxw=CW,
...
def _estimate_pages(lineas: list) -> int:
    ...
    first = _capacity(lines_top, MB + SUM_H + 4)
    more  = _capacity(lines_top, MB + 4)
    n = len(lineas)
    if n <= first:
        return 1
...
def generar_pdf(data: dict) -> bytes:
    ...
    while True:
        ...
        is_last      = len(remaining) <= _capacity(lines_top, MB + SUM_H + 4)
        lines_bottom = (MB + SUM_H + 4) if is_last else MB + 4

        _draw_header(c, cab, emp, page, total_pages)
        _, remaining = _draw_lines(c, remaining, lines_top, lines_bottom)

        if is_last:
            _draw_summary(c, cab, venc, emp, MB)
