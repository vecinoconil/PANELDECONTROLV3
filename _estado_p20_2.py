...
print("=== ESTADO ANTES del cobro: P 20-2 (id=66236) ===")
...
""", (idcab,))
r = cur.fetchone()
print(f"  total={float(r['total']):.2f}  cobrado={float(r['cobrado']):.2f}  pendiente={float(r['total'])-float(r['cobrado']):.2f}  num_ventas_entregas={r['num_ve']}")

print("\n--- ventas_entregas ---")
cur.execute("SELECT id, importe, fecha, idregistro, cajabanco, codigo_cb, usuario, turno FROM ventas_entregas WHERE idcab = %s ORDER BY id", (idcab,))
...
print("\n--- IDs máximos actuales (para comparar después) ---")
